...
def main():
    parser = argparse.ArgumentParser(
        description="Parse TST file and output to XLSX")
    ...
    # 构建输出 XLSX 文件名
    output_file_name = f"{input_file_name}.xlsx"
    ...
    header_row = ['name', 'subprogram', 'srb_num', 'test_method', 'export_test_method',
                  'combined_values', 'expected_value']
    worksheet.append(header_row)

    for test_case in test_cases:
        row_data = [test_case.get(field) for field in header_row]
...
# pyinstaller -F D:/Python_WorkSpace/tst_to_xlsx.py -w 